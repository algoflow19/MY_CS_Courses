...
class Node(object):
    ...
    def __add__(self,str_item):
        if(type(str_item)==type("")):
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        if( not ( edge.src in self.nodes and edge.dest in self.nodes)):
            raise ValueError("Add edge that has node(s) not in the graph!"+str(edge))
        if(self.edges.get(edge.get_source(),-1)!=-1):
